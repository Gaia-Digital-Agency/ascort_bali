...
servicesRouter.post("/", requireAuth, requireRole(["provider","admin"]), async (req: AuthedRequest, res) => {
...
servicesRouter.put("/:id", requireAuth, requireRole(["provider","admin"]), async (req: AuthedRequest, res) => {
  const id = req.params.id;
  const parsed = UpsertSchema.safeParse(req.body);
  if (!parsed.success) return res.status(400).json({ error: "invalid_body", details: parsed.error.flatten() });
